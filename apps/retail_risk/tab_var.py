"""Tab 5 — VaR & Greeks: risk metrics including load uncertainty VaR component."""
...
def render_var(engine):
    ...
    # Load Uncertainty VaR
    st.divider()
    st.subheader("Load Uncertainty VaR")
    _render_load_uncertainty_var(book_id, engine)
    ...
    c1, c2 = st.columns(2)
    with c1:
        spot_shock = st.slider("Spot price shock (%)", -50, 50, 0, key="retail_spot_shock")
    with c2:
        bilateral_shock = st.slider("Bilateral benchmark shock (%)", -50, 50, 0, key="retail_bi_shock")
    ...
    if spot_shock != 0 or bilateral_shock != 0:
        ...
        if not stress_pos.empty:
            ...
            st.metric(
                "Scenario P&L Impact", f"¥{scenario_pnl:,.0f}",
                delta_color="inverse" if scenario_pnl < 0 else "normal"
            )


def _render_load_uncertainty_var(book_id: int, engine):
    """Estimate VaR component from retail load uncertainty."""
    with engine.connect() as conn:
        profiles = pd.read_sql(text("""
            SELECT cp.profile_date, SUM(cp.load_mwh) AS total_load,
                   SUM(cp.nominated_mwh) AS total_nominated
    ...
    if profiles.empty or len(profiles) < 10:
        st.info("Need 10+ days of customer profile data for load uncertainty VaR.")
        return

    # Load deviation = actual load - nominated
    profiles["load_dev"] = profiles["total_load"] - profiles["total_nominated"]
    load_sigma = float(profiles["load_dev"].std())
    avg_load = float(profiles["total_load"].mean())

    # Get average market price from position volumes
    with engine.connect() as conn:
        avg_price_row = conn.execute(text("""
            SELECT AVG(market_price_cny_mwh) FROM marketdata.rm_position_volumes
            WHERE book_id = :bid AND market_price_cny_mwh IS NOT NULL
        """), {"bid": book_id}).scalar()

    avg_price = float(avg_price_row or 300.0)

    # Load uncertainty VaR (1D, 95%) = 1.645 * sigma_load * avg_price
    load_var_95 = 1.645 * load_sigma * avg_price
    load_var_99 = 2.326 * load_sigma * avg_price

    col1, col2, col3 = st.columns(3)
    col1.metric("Avg Daily Load", f"{avg_load:,.1f} MWh")
    col2.metric("Load Uncertainty 1D 95% VaR", f"¥{load_var_95:,.0f}")
    col3.metric("Load Uncertainty 1D 99% VaR", f"¥{load_var_99:,.0f}")

    # Histogram of load deviations
    fig = go.Figure()
    fig.add_trace(go.Histogram(
        x=profiles["load_dev"], nbinsx=20,
        marker_color="#3498db", name="Load Deviation"
    ))
    fig.update_layout(
        title="Distribution of Daily Load Deviation (Actual - Nominated)",
        xaxis_title="MWh", yaxis_title="Count", height=280
    )
