...
def _parse_table_rows(page, provinces_cn: List[str], mode: str) -> List[Dict]:
    ...
    for tbl in tables:
        ...
        for raw_row in tbl:
            ...
            header_markers = frozenset({"省份", "均价", "最高价", "最低价", "出清均价", "环比"})
            ...
            if mode == "RT":
                out.append({"province_cn": prov_cn, "rt_avg": avg, "rt_max": mx, "rt_min": mn})
            else:  # "DA"
                out.append({"province_cn": prov_cn, "da_avg": avg, "da_max": mx, "da_min": mn})
