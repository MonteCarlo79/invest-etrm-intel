...
try:
    ...
    # Accept both bare decorator (@tool) and parametrized form (@tool(...))
...
class Candidate(BaseModel):
    model_config = ConfigDict(extra='forbid')
    hours: List[float] = Field(min_items=1)
    ...
    for p in provs:
        ...
        if best_i >= 0:
            ...
            assigned[p.prov] = AssignedOne(
                hours=c.hours,
                rmse=0.0,
                within_tolerance=True,  # TODO: compute real RMSE if needed
                page=c.page,
                box=c.box,
...
__all__ = ["validate_against_summary",
