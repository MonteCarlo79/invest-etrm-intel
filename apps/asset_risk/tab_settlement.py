...
def render_settlement(engine):
    ...
    st.divider()
    st.subheader("Merchant Exposure · 市场化收入占比")
    _render_merchant_share(book_id, engine)
...
def _process_pdf(uploaded, book_id: int, settlement_month, engine, file_hash: str | None = None,
    ...
    try:
        if voucher_fallback:
            ...
            if is_scanned:
                # Vision+verify path: accept the extraction ONLY when every page
                # reconciles with its printed 电费合计 (fee identity), else refuse.
                from services.settlement_ingest.parser_scan_charge import parse_scanned_charge_bill_vision
                items = parse_scanned_charge_bill_vision(tmp_path)
                if not items:
                    st.warning(
                        f"**{uploaded.name}** looks like a 下网 charge bill but is a scanned image — "
                        "the scan parser could not verify its extraction against the printed 电费合计. "
                        "Skipped: no data written. Send it to Claude for hand-processing or obtain the text version."
                    )
                    return
                st.info("Scanned 电费计算明细 — vision extraction verified against printed totals.")
            else:
...
def _year_subtotal_row(pivot, monthly, year, days_in_month, energy_per_cycle_mwh,
    ...
    if is_wind and capacity_mw and capacity_mw > 0:
        ...
        row["LF"] = total_discharge_vol / (capacity_mw * yr_days * 24) if yr_days > 0 else 0
...
def _render_analytics(book_id: int, engine):
    ...
    if is_wind:
        # 发电小时数 = full-load hours (发电量/装机容量); LF = 发电量/装机容量/elapsed hours
        col5.metric("发电小时数", f"{revenue_vol / capacity_mw:,.0f} h" if capacity_mw else "N/A")
        col6.metric("LF", f"{revenue_vol / (capacity_mw * elapsed_hours):.1%}"
    ...
    # Add 电能转化率 per month (BESS) — meaningless for wind; show 发电小时数 + LF
    # (LF = monthly generation / installed capacity / hours in month)
    import numpy as np
    if is_wind and capacity_mw and capacity_mw > 0:
        pivot["发电小时数"] = (discharge_vol.values / capacity_mw).round(1)
        month_hours = pd.Series(days_in_month, index=pivot.index).values * 24
        pivot["LF"] = (discharge_vol.values / (capacity_mw * month_hours))
    else:
    ...
    desired_order = [
        ...
        "度电总价差", "容量补偿价差", "套利价差", "发电小时数", "LF", "日均充放次数", "转化率",
    ]
...
# ── Merchant Exposure (市场化收入占比) ────────────────────────────────────────

_MERCHANT_CATS = ("discharge_energy", "generation_revenue")
_CONTRACT_CATS = ("capacity_compensation", "subsidy", "rebate", "other")
_SERVICE_CATS = ("frequency",)
_COST_CATS = ("charge_energy", "transmission", "system_operation",
              "coal_capacity_charge", "basic_fee", "govt_surcharges", "penalty")


def _bucket_of(category: str) -> str:
    """Map a settlement category to a merchant-exposure bucket."""
    if category in _MERCHANT_CATS:
        return "merchant"
    if category in _CONTRACT_CATS:
        return "contract"
    if category in _SERVICE_CATS:
        return "services"
    return "cost"


def compute_merchant_shares(items_df):
    """Per-month bucket sums + merchant share of gross inflows.

    items_df columns: category, month (YYYY-MM), amount_cny.
    merchant share = merchant / (merchant + contract + services) bucket sums,
    where bucket sums are netted within the bucket for the month and floored at 0
    for the inflow denominator. Returns (monthly_df, latest_month_df or None).
    """
    if items_df.empty:
        return pd.DataFrame(), None
    df = items_df.copy()
    df["bucket"] = df["category"].map(_bucket_of)
    monthly = (
        df[df["bucket"].isin(["merchant", "contract", "services"])]
        .groupby(["month", "bucket"], as_index=False)["amount_cny"].sum()
        .pivot(index="month", columns="bucket", values="amount_cny")
        .fillna(0.0)
    )
    for col in ("merchant", "contract", "services"):
        if col not in monthly.columns:
            monthly[col] = 0.0
    monthly["gross"] = (monthly["merchant"].clip(lower=0)
                        + monthly["contract"].clip(lower=0)
                        + monthly["services"].clip(lower=0))
    monthly["merchant_share"] = monthly.apply(
        lambda r: (r["merchant"] / r["gross"]) if r["gross"] > 1e-9 else None,
        axis=1,
    )
    monthly = monthly.reset_index().sort_values("month")
    latest = monthly.dropna(subset=["merchant_share"])
    return monthly, (latest.iloc[-1] if not latest.empty else None)


def _render_merchant_share(book_id: int, engine):
    """Merchant (spot) vs contract/mechanism revenue decomposition per book.

    Ardian Opta lesson: track how much of each asset's revenue is exposed to the
    merchant market vs regulated/contracted streams — the 136号文 exposure metric.
    """
    import plotly.graph_objects as go

    with engine.connect() as conn:
        items_df = pd.read_sql(text("""
            SELECT si.category, si.amount_cny,
                   to_char(s.settlement_month::date, 'YYYY-MM') AS month
            FROM marketdata.rm_settlement_items si
            JOIN marketdata.rm_settlements s ON s.id = si.settlement_id
            WHERE s.book_id = :bid
        """), conn, params={"bid": book_id})
    ...
    if items_df.empty:
        st.info("暂无结算数据——上传结算单后此处显示市场化/合约收入分解。")
        return

    monthly, latest = compute_merchant_shares(items_df)
    if latest is None:
        st.info("结算数据不足以计算市场化占比。")
        return

    # ── Headline tiles ──────────────────────────────────────────────────────
    share = latest["merchant_share"]
    contract_share = (latest["contract"] / latest["gross"]) if latest["gross"] > 1e-9 else 0.0
    services_share = (latest["services"] / latest["gross"]) if latest["gross"] > 1e-9 else 0.0
    c1, c2, c3 = st.columns(3)
    c1.metric(f"市场化（现货）占比 · {latest['month']}", f"{share:.1%}")
    c2.metric("合约/机制占比", f"{contract_share:.1%}")
    c3.metric("辅助服务占比", f"{services_share:.1%}")

    if share >= 0.7:
        st.caption("🔴 高现货敞口 — 收入主要随市场价格波动（136号文 merchant 暴露大）")
    elif share >= 0.4:
        st.caption("🟡 中等现货敞口 — 现货与合约收入混合")
    else:
        st.caption("🟢 低现货敞口 — 收入主要由合约/机制锚定")

    # ── Monthly trend ────────────────────────────────────────────────────────
    trend = monthly.dropna(subset=["merchant_share"])
    if len(trend) >= 2:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=trend["month"], y=trend["merchant_share"] * 100,
            mode="lines+markers", name="市场化占比 (%)",
            line=dict(color="#C62828", width=2),
        ))
        fig.update_layout(height=260, margin=dict(t=10, b=10),
                          yaxis_title="市场化收入占比 (%)", xaxis_title="")
        fig.add_hline(y=70, line_dash="dot", line_color="red", opacity=0.4)
        fig.add_hline(y=40, line_dash="dot", line_color="green", opacity=0.4)
        st.plotly_chart(fig, use_container_width=True)

    # ── Latest-month decomposition ───────────────────────────────────────────
    m = latest
    fig2 = go.Figure()
    for key, label, color in (
        ("merchant", "市场化（现货）", "#C62828"),
        ("contract", "合约/机制", "#1565C0"),
        ("services", "辅助服务", "#7B1FA2"),
    ):
        fig2.add_bar(x=[m["month"]], y=[m[key]], name=label, marker_color=color)
    fig2.update_layout(barmode="stack", height=280, margin=dict(t=10, b=10),
                       yaxis_title="金额 (元)", xaxis_title="",
                       title=f"{m['month']} 收入分解（成本扣减项见结算明细）")
    st.plotly_chart(fig2, use_container_width=True)
    st.caption(
        "口径：市场化=放电收入/发电收入；合约=容量补偿+补贴+退补+其他；辅助服务=调频。"
        "占比为三者毛流入之比；充电电费、输配/系统运行/基金/偏差等扣减项不计入分母。"
    )
