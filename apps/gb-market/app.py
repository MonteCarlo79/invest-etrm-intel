...
def _gb_kb_stats() -> dict:
    """Return GB KB + insight statistics for the metrics dashboard."""
    ...
    docs_row = _query(
        "SELECT COUNT(*)::int AS n, MAX(fetched_at) AS last_fetch "
        "FROM intl_market.gb_knowledge_docs"
    )
    insights_row = _query(
        "SELECT COUNT(*)::int AS n FROM intl_market.gb_expert_insights WHERE active = TRUE"
    )
    digest_row = _query(
        """SELECT COUNT(DISTINCT d.id)::int AS digested,
               (SELECT COUNT(*)::int FROM intl_market.gb_knowledge_docs) AS total
            FROM intl_market.gb_knowledge_docs d
            WHERE EXISTS (
                SELECT 1 FROM intl_market.gb_expert_insights i
                WHERE i.source_doc_url = d.url
            )"""
    )
    sched_row = _query(
        """SELECT COUNT(*) FILTER (WHERE status = 'success')::int AS successes,
