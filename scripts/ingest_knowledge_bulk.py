...
def _collect_files(root: Path, allowed_exts: set[str]) -> list[Path]:
...
def _ingest_file(
    ...
    try:
        ...
        if is_new:
            return "added", f"{path.name}  [category={category}  doc_id={doc_id}]"
        else:
...
def main() -> None:
    ...
    # Discover files
    files = _collect_files(root, allowed_exts)
    if not files:
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(_ingest_file, f, args.category, api_key, False): f
            for f in files
        }
        done = 0
