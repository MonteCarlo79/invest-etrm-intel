...
def evaluate(engine: Engine, schema: str = "marketdata",
    ...
    if window_end is None:
        # Anchor on the latest valid day, never a future-dated junk row
        # (prod had a 2026-12-31 artifact row that emptied the first seed).
        from datetime import date as _date
        window_end = min(capture["date"].max().date(), _date.today())
