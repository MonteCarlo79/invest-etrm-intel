"""CAISO data ingestion from Modo Energy API → intl_market.caiso_* tables.

Usage:
    python -m services.modo_energy.caiso_ingestion --start 2024-01-01 --end 2026-05-10
"""
import argparse
import os
import sys
...
_DDL = {
    "intl_market.caiso_bess_assets": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_bess_assets (
    ...
    """,
    "intl_market.caiso_bess_daily_index": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_bess_daily_index (
    ...
    """,
    "intl_market.caiso_bess_monthly_index": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_bess_monthly_index (
            year_month TEXT NOT NULL, market TEXT NOT NULL,
            revenue_permw NUMERIC, revenue_permwh NUMERIC, duration NUMERIC,
            PRIMARY KEY (year_month, market)
        )
    """,
    "intl_market.caiso_bess_leaderboard": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_bess_leaderboard (
    ...
    """,
    "intl_market.caiso_spot_price": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_spot_price (
            settlement_date DATE NOT NULL, settlement_period INT NOT NULL,
            region TEXT NOT NULL DEFAULT 'CAISO', spot_price NUMERIC,
            PRIMARY KEY (settlement_date, settlement_period, region)
        )
    """,
    "intl_market.caiso_ancillary_results": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_ancillary_results (
            settlement_date DATE NOT NULL, service TEXT NOT NULL,
            region TEXT NOT NULL DEFAULT 'CAISO',
            clearing_price NUMERIC, volume_mw NUMERIC,
            PRIMARY KEY (settlement_date, service, region)
        )
    """,
    "intl_market.caiso_ingestion_log": """
        CREATE TABLE IF NOT EXISTS intl_market.caiso_ingestion_log (
...
def _try_get(client, path, params=None):
    try:
        ...
        if any(c in str(exc) for c in ["404", "403", "400"]):
            print(f"  [caiso] endpoint {path} not available: {exc}")
            return None
        raise
...
def run_ingestion(start, end, only=None):
    ...
    def ingest_assets():
        records = _try_get(client, "/caiso/modo/asset/database")
        if not records:
            return 0
        ...
        return _upsert(engine, "intl_market.caiso_bess_assets",
    ...
    def ingest_leaderboard():
        ...
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/caiso/modo/benchmarking/leaderboard-live",
            ...
            rows = [{"asset": r.get("asset_name", r.get("asset", "")),
                    ...
                    for r in records]
            total += _upsert(engine, "intl_market.caiso_bess_leaderboard", pd.DataFrame(rows),
    ...
    def ingest_daily_index():
        total = 0
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/caiso/modo/benchmarking/daily-index-live",
                               {"date_from": d_from.isoformat(), "date_to": d_to.isoformat()})
            if not records:
                continue
            rows = [{"settlement_date": r.get("date", r.get("settlement_date")),
                    ...
                    for r in records]
            total += _upsert(engine, "intl_market.caiso_bess_daily_index", pd.DataFrame(rows),
    ...
    def ingest_spot_price():
        total = 0
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/caiso/modo/markets/lmp-live",
                               {"date_from": d_from.isoformat(), "date_to": d_to.isoformat()})
            if not records:
                continue
            rows = [{"settlement_date": r.get("date", r.get("settlement_date")),
                     "settlement_period": r.get("settlement_period", r.get("period", 1)),
                     "region": r.get("region", "CAISO"), "spot_price": r.get("lmp", r.get("price"))}
                    for r in records]
            total += _upsert(engine, "intl_market.caiso_spot_price", pd.DataFrame(rows),
    ...
    for key, fn in [("assets", ingest_assets), ("leaderboard", ingest_leaderboard),
        ...
        if only and key not in only:
            continue
        print(f"  [caiso/{key}]…", end="", flush=True)
        try:
