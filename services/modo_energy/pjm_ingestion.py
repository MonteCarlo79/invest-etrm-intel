"""PJM data ingestion from Modo Energy API → intl_market.pjm_* tables.

Usage:
    python -m services.modo_energy.pjm_ingestion --start 2024-01-01 --end 2026-05-10
"""
import argparse
import os
import sys
...
_DDL = {
    "intl_market.pjm_bess_assets": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_bess_assets (
    ...
    """,
    "intl_market.pjm_bess_daily_index": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_bess_daily_index (
    ...
    """,
    "intl_market.pjm_bess_monthly_index": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_bess_monthly_index (
            year_month TEXT NOT NULL, market TEXT NOT NULL,
            revenue_permw NUMERIC, revenue_permwh NUMERIC, duration NUMERIC,
            PRIMARY KEY (year_month, market)
        )
    """,
    "intl_market.pjm_bess_leaderboard": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_bess_leaderboard (
    ...
    """,
    "intl_market.pjm_spot_price": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_spot_price (
            settlement_date DATE NOT NULL, settlement_period INT NOT NULL,
            region TEXT NOT NULL DEFAULT 'PJM', spot_price NUMERIC,
            PRIMARY KEY (settlement_date, settlement_period, region)
        )
    """,
    "intl_market.pjm_ancillary_results": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_ancillary_results (
    ...
    """,
    "intl_market.pjm_ingestion_log": """
        CREATE TABLE IF NOT EXISTS intl_market.pjm_ingestion_log (
...
def _try_get(client, path, params=None):
    try:
        ...
        if any(c in str(exc) for c in ["404", "403", "400"]):
            print(f"  [pjm] endpoint {path} not available: {exc}")
            return None
        raise
...
def run_ingestion(start, end, only=None):
    ...
    def ingest_assets():
        records = _try_get(client, "/pjm/modo/asset/database")
        if not records:
            return 0
        ...
        return _upsert(engine, "intl_market.pjm_bess_assets", pd.DataFrame(rows) if rows else pd.DataFrame(),
                       ["asset", "history_table", "date_from"])

    def ingest_leaderboard():
        total = 0
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/pjm/modo/benchmarking/leaderboard-live",
            ...
            rows = [{"asset": r.get("asset_name", r.get("asset", "")),
                    ...
                    for r in records]
            total += _upsert(engine, "intl_market.pjm_bess_leaderboard", pd.DataFrame(rows),
    ...
    def ingest_daily_index():
        total = 0
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/pjm/modo/benchmarking/daily-index-live",
                               {"date_from": d_from.isoformat(), "date_to": d_to.isoformat()})
            if not records:
                continue
            rows = [{"settlement_date": r.get("date", r.get("settlement_date")),
                    ...
                    for r in records]
            total += _upsert(engine, "intl_market.pjm_bess_daily_index", pd.DataFrame(rows),
    ...
    def ingest_spot_price():
        total = 0
        for d_from, d_to in _chunk_dates(start, end):
            records = _try_get(client, "/pjm/modo/markets/lmp-live",
                               {"date_from": d_from.isoformat(), "date_to": d_to.isoformat()})
            if not records:
                continue
            rows = [{"settlement_date": r.get("date", r.get("settlement_date")),
                     "settlement_period": r.get("settlement_period", r.get("period", 1)),
                     "region": r.get("region", "PJM"), "spot_price": r.get("lmp", r.get("price"))}
                    for r in records]
            total += _upsert(engine, "intl_market.pjm_spot_price", pd.DataFrame(rows),
    ...
    for key, fn in [("assets", ingest_assets), ("leaderboard", ingest_leaderboard),
        ...
        if only and key not in only:
            continue
        print(f"  [pjm/{key}]…", end="", flush=True)
        try:
