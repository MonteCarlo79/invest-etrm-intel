...
class ThinkingAgent:
    ...
    def _build_design_prompt(self) -> str:
        ...
        return (
            ...
            f"本周审查范围：{apps_str}\n\n"
            "**关键表结构（查询时务必使用正确列名）**\n"
            "hermes.thinking_log 列：id, mode, ts, files_read, tables_checked, message_sent, model_used\n"
            "  -- 查重用 message_sent，不要用 content\n"
            "marketdata.md_da_cleared_energy 列：province, trade_date, period_start, period_end, "
            "cleared_energy_mwh, cleared_price_cny_mwh, inserted_at\n"
            "marketdata.md_id_cleared_energy 列：province, trade_date, trade_interval, node_name, "
            "cleared_energy_mwh, cleared_price_cny_mwh, inserted_at\n\n"
            "步骤：\n"
            "1. 对每个应用，先调用 `list_app_files` 查看文件列表。\n"
            "2. 用 `read_source_file` 阅读关键文件（最多共读 5 个文件）。\n"
            "3. 用 `query_db` 了解 DB 中有哪些数据。\n"
            "4. 查看 hermes.thinking_log 最近14天的记录，避免重复已发送的观察：\n"
            "   SELECT message_sent, ts FROM hermes.thinking_log "
            "WHERE mode = 'design' AND ts > NOW() - INTERVAL '14 days' ORDER BY ts DESC LIMIT 5\n"
