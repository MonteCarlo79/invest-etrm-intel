...
# Startup-log sentinel: print connection mode once per process, never print secrets.
_DB_MODE_LOGGED = False


def _db_engine():
    """Return a SQLAlchemy engine using the first available DB config:
    1. DB_DSN  (e.g. postgresql://user:pass@host:5432/db)
    2. PGURL   (same format; injected by ECS common_env)
    3. discrete DB_USER / DB_PASSWORD / DB_HOST / DB_PORT / DB_NAME env vars
    """
    ...
    if not _DB_MODE_LOGGED:
        # Extract host only — never log user/password.
        try:
            from urllib.parse import urlparse
            host = urlparse(url).hostname or DB_DEFAULTS["host"]
        except Exception:
            host = DB_DEFAULTS["host"]
        print(f"[column_to_matrix_all] db connection mode={mode} host={host}", flush=True)
