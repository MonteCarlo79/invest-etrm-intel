"""ERCOT knowledge base ingestion orchestrator.

Sources:
  - Modo AI (Playwright distillation) — primary daily intelligence
  - ERCOT market notices (public API/RSS)

Usage:
    python -m services.ercot_knowledge.ingest
    python -m services.ercot_knowledge.ingest --only modo_ai
"""
import argparse
import os
import sys
...
def run_knowledge_ingest(only: list[str] | None = None, verbose: bool = True) -> dict[str, int]:
    ...
    from services.ercot_knowledge.config import MARKET_CONFIG
    ...
    try:
        ...
        connectors.append(("modo_ai", "Modo Energy AI (ERCOT distillation)", ModoAIConnector(MARKET_CONFIG)))
