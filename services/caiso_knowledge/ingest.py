"""CAISO knowledge base ingestion orchestrator.

Sources:
  - Modo AI (Playwright distillation) — primary daily intelligence
  - CAISO market notices (public)

Usage:
    python -m services.caiso_knowledge.ingest
"""
import argparse
import os
import sys
...
def run_knowledge_ingest(only: list[str] | None = None, verbose: bool = True) -> dict[str, int]:
    ...
    from services.caiso_knowledge.config import MARKET_CONFIG
    ...
    try:
        ...
        connectors.append(("modo_ai", "Modo Energy AI (CAISO distillation)", ModoAIConnector(MARKET_CONFIG)))
