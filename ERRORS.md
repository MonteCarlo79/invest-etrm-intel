...
## Fargate CannotPullContainerError from Mac builds: image is arm64, Fargate needs linux/amd64 (2026-09-09)

**What didn't work:**
- Plain `docker build -f apps/asset_risk/Dockerfile -t bess-asset-risk:v65 .` on the Apple-Silicon MacBook → pushed, jq-swapped into td:67 → deployment stuck IN_PROGRESS 10+ min: `CannotPullContainerError: image Manifest does not contain descriptor matching platform 'linux/amd64'`. Default `docker build` on arm64 Mac emits an arm64-only manifest; ECS retries placement forever. Old task keeps serving throughout — no outage, but the rollout never lands.

**What worked:**
- Rebuild with `docker build --platform linux/amd64 ...` (QEMU; re-runs uncached layers slowly), re-push the same tag — the IN_PROGRESS deployment's next placement retry succeeds on its own, no second update-service needed.
- Verify platform BEFORE update-service: `docker buildx imagetools inspect <ecr-uri>:<tag> | grep Platform` must list `linux/amd64` (an `unknown/unknown` attestation entry alongside is normal BuildKit output).
- Verify code actually inside the image: `docker run --rm --platform linux/amd64 --entrypoint grep <image> -c "<unique-string>" /app/<path>`.
- Root cause of the full rebuild: `python:3.11-slim` base had moved (3.11.16), busting cache for the 881MB pip layer.

## ECR push broken pipe on large layers (Mac, no system proxy) (2026-09-09)

**What didn't work:**
1. `docker push` of a ~300MB-compressed layer died 3× with `write tcp ...->192.168.65.1:3128: write: broken pipe`. 192.168.65.1:3128 is Docker Desktop's internal forwarder (`http.docker.internal`); macOS has no system proxy. Small layers and all `aws` CLI calls succeed — only large sustained uploads get killed (same flakiness as github.com being unreachable the day before).
2. `docker push ... | tail -2` masks failures: pipeline exit status is tail's, so `&&`-chained steps (e.g. a second push) run even after "failed to copy". Push unpiped, or `set -o pipefail`.
